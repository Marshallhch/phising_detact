...
def train(csv_path: str, model_out: str):
    ...
    # train_test_split(입력데이터, 타겟데이터, 테스트 사이즈, 랜덤값 조정, 클래스 비율 유지 코드)
    X_train, X_test, y_train, y_test = train_test_split(df['text'], df['label'], test_size=0.2, random_state=42, stratify=df['label'].values)
    ...
    pipeline = Pipeline([
        ...
        ('tfidf', TfidfVectorizer(
            ...
            max_features=10000, # 특성을 1만개만 사용(메모리 효율)
